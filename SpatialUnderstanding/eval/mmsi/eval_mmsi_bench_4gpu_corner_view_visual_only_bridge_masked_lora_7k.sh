#!/bin/bash
#SBATCH --job-name=mmsi_cv_bm_lora_7k
#SBATCH --output=/path/to/ThinkMorph-BAGEL-release/SpatialUnderstanding/mmsi_log/mmsi_corner_view_visual_only_bridge_masked_lora_7k_batch%a_output.txt
#SBATCH --error=/path/to/ThinkMorph-BAGEL-release/SpatialUnderstanding/mmsi_log/mmsi_corner_view_visual_only_bridge_masked_lora_7k_batch%a_error.txt
#SBATCH --ntasks=1
...
# MMSI-Bench inference for cv_bm_lora_7k
...
OUTPUT_DIR="${OUTPUT_DIR:-/path/to/scratch/VisualCoT/eval_outputs/BAGEL_format_corner_view_visual_only_bridge_masked_lora_7k_mmsi}"
...
echo "MMSI-Bench cv_bm_lora_7k — Model: $MODEL_PATH | Shards: $NUM_SHARDS | Batch: $SHARD_BATCH (covers ${SHARD_INDICES[*]})"
