#!/bin/bash
#SBATCH --job-name=mmsi_vanilla_bagel_default
#SBATCH --output=/path/to/ThinkMorph-BAGEL-release/SpatialUnderstanding/mmsi_log/mmsi_vanilla_bagel_default_4gpu_output_%a.txt
#SBATCH --error=/path/to/ThinkMorph-BAGEL-release/SpatialUnderstanding/mmsi_log/mmsi_vanilla_bagel_default_4gpu_error_%a.txt
#SBATCH --ntasks=1
...
#
# MMSI-Bench Inference — vanilla BAGEL-7B-MoT, think=true, bagel_default
# No eval-side system prompt; rely on BAGEL's own internal default prompt
# (activated by --think). 4 GPUs × 4 shards on short-unkillable (3 h cap).
#
...
OUTPUT_DIR="${OUTPUT_DIR:-/path/to/scratch/VisualCoT/BAGEL_vanilla_mmsi_bagel_default_eval}"
...
THINKING_MODE="${THINKING_MODE:-bagel_default}"
...
echo "=============================================="
echo "MMSI-Bench Inference — vanilla BAGEL, think=true, bagel_default"
echo "=============================================="
