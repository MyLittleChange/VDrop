#!/bin/bash
#SBATCH --job-name=fnvt_direction_visual_only_
#SBATCH --output=/path/to/ThinkMorph-BAGEL-release/SpatialUnderstanding/bagel/bagel_spatial_4gpu_direction_visual_only_bridge_masked_lora_7k_force_no_vt_output.txt
#SBATCH --error=/path/to/ThinkMorph-BAGEL-release/SpatialUnderstanding/bagel/bagel_spatial_4gpu_direction_visual_only_bridge_masked_lora_7k_force_no_vt_error.txt
#SBATCH --ntasks=1
...
#
# force_no_visual_thinking variant.
# Task: direction | Model: visual_only_bridge_masked_lora_7k
# Same visual_only_thinking system prompt as the Setting C rows, but the
# inferencer hard-skips gen_image / bridge-KV append / <image_start>/<image_end>
# wrapper. Answer is decoded directly from V1 + V2 + question. Companion
# ablation that isolates the bridge-generation forward pass itself from the
# bridge-KV-blinding step.
#
set -euo pipefail
...
DATASET_FILES="${DATASET_FILES:-approved_mcqs_relative_direction_normalized.json}"

OUTPUT_DIR="${OUTPUT_DIR:-/path/to/scratch/VisualCoT/BAGEL_format_mix_all_balance_visual_only_bridge_masked_lora_7k_mcqs_relative_direction_normalized_force_no_vt}"
...
echo "=============================================="
echo "BAGEL Spatial - force_no_visual_thinking (direction)"
echo "Model: visual_only_bridge_masked_lora_7k"
echo "=============================================="
