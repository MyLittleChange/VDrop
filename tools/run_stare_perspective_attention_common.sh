...
# Shared implementation for STARE perspective answer-token attention Slurm
# launchers. Wrapper scripts set GPUS_PER_JOB, RUN_SET, and Slurm resources.
...
OUTPUT_ROOT="${OUTPUT_ROOT:-/path/to/scratch/VisualCoT/artifacts/stare_perspective_attention_test}"
SAMPLE_COUNT="${SAMPLE_COUNT:-all}"
MANIFEST="${MANIFEST:-$OUTPUT_ROOT/fixed_stare_perspective_test_n${SAMPLE_COUNT}.json}"
...
if [[ ! -f "$MANIFEST" ]]; then
  echo "Creating STARE fixed manifest: $MANIFEST"
  "$PYTHON" "$REPO_ROOT/tools/create_stare_perspective_manifest.py" \
    --data_file "$DATA_FILE" \
    --sample_count "$SAMPLE_COUNT" \
    --output "$MANIFEST"
else
  echo "Reusing STARE fixed manifest: $MANIFEST"
fi
...
echo "STARE perspective attention probe"
