#!/bin/bash
#SBATCH --job-name=annotate_point_matching_text_reasoning
#SBATCH --partition=short-unkillable
#SBATCH --gres=gpu:a100l:4
#SBATCH -c 24
#SBATCH --output=/path/to/ThinkMorph-BAGEL-release/SpatialUnderstanding/data_creation/training_data/slurm_logs/annotate_point_matching_text_reasoning-%A_%a.txt
#SBATCH --error=/path/to/ThinkMorph-BAGEL-release/SpatialUnderstanding/data_creation/training_data/slurm_logs/annotate_point_matching_text_reasoning_error-%A_%a.txt
#SBATCH --ntasks=1
#SBATCH --time=3:00:00
#SBATCH --mem=512Gb

# ==============================================================================
# Text-reasoning annotation for the point-matching no_thinking jsonl.
#
# Mirrors run_annotate_text_reasoning.sh: starts Qwen3-VL-235B-FP8 on this
# allocation, waits for the local OpenAI-compatible endpoint, runs
# annotate_point_matching_text_reasoning.py, then stops the server on exit.
#
# Usage:
#   /opt/slurm/bin/sbatch \
#       SpatialUnderstanding/data_creation/training_data/run_annotate_point_matching_text_reasoning.sh
#   /opt/slurm/bin/sbatch --array=0-7 ...
#
# Useful overrides:
...
INPUT="${INPUT:-/path/to/scratch/infinigen/training_data_point_matching/no_thinking/no_thinking.jsonl}"
OUTPUT_DIR="${OUTPUT_DIR:-/path/to/scratch/infinigen/training_data_point_matching/text_thinking}"

VLLM_LOG="${LOG_DIR}/annotate_point_matching_text_reasoning_vllm-${SLURM_JOB_ID:-local}.log"
SERVER_PID=""
...
log "Starting point-matching text-reasoning annotation with API_BASE=$API_BASE"
...
python "$SCRIPT_DIR/annotate_point_matching_text_reasoning.py" "${ANNOTATE_ARGS[@]}"

log "Point-matching text-reasoning annotation complete."
