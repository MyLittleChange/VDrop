...
two perspective images + a panorama view.
...
class GeminiThreeImageInference:
    ...
    def run_single_sample(
        ...
        system_prompt = (
            ...
            "using the panoramic view to understand the overall room layout and how the two viewpoints relate to each other, "
...
def main():
    parser = argparse.ArgumentParser(
        description="Run inference on Spatial Collaboration dataset using Gemini API with 2 images + panorama view",
        epilog="Note: Set VisualCoT_GEMINI environment variable.",
    )
    parser.add_argument(
        "--dataset_file",
        default="/path/to/scratch/VisualCoT/spatial_collab_dataset/approved_mcqs_relative_direction_normalized.json",
        help="Path to the dataset JSON file (must have panorama_path field)",
    )
    parser.add_argument(
        "--model_name",
    ...
    parser.add_argument(
        "--output_file",
        default="/path/to/scratch/spatial_collab/Gemini/inference_results_gemini_panorama.json",
    )
    ...
    # Filter to samples that have a panorama
    valid_data = []
    missing_panorama = 0
    ...
    def load_images(example: Dict[str, Any]):
        """Load all three images: user_1, user_2, and panorama."""
    ...
    def process_single_sample(example: Dict[str, Any]):
        ...
        try:
            ...
            result["panorama_path"] = loaded_data["panorama_path"]
