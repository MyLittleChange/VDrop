#!/usr/bin/env python3
"""
Test BAGEL model on center-view generation task.

Supports three thinking modes (matching training):
  --thinking_mode interleaved_thinking  (default)
    System prompt asks for text + visual reasoning.
    Model generates text (<think>...</think>) and image (<image_start>...<image_end>).

  --thinking_mode text_only_thinking
    System prompt asks for text reasoning only.
    Model generates text (<think>...</think>), no image generation.

  --thinking_mode visual_only_thinking
    System prompt asks for visual reasoning only.
    Model generates image (<image_start>...<image_end>), no text reasoning.
"""
...
def main():
    parser = argparse.ArgumentParser(
        description="Test BAGEL on center-view generation task"
    )
    parser.add_argument(
        "--annotation_file",
        default="/path/to/scratch/VisualCoT/annotations/gemini_3_flash_preview_rendered_v3_anchor_annotations_center_view_no_angle.json",
    )
    parser.add_argument(
        "--model_path",
    ...
    parser.add_argument(
        "--output_dir",
        default="/path/to/scratch/spatial_collab/BAGEL/center_view_test",
    )
    ...
    parser.add_argument(
        "--omit_center_view",
        action="store_true",
        help="Skip center view ground truth loading (for datasets without GT center views)",
    )
    parser.add_argument(
        "--shard",
        type=str,
    ...
    # text_only_thinking is understanding-only (no image generation)
    ...
    print(f"Loaded {len(results)} annotation results")

    # Detect parsed_qa format from filename
    is_parsed_qa = "parsed_qa" in os.path.basename(args.annotation_file)
    if is_parsed_qa:
        print("Detected parsed_qa annotation format (using 'question' field for instruction)")

    # Filter out samples with empty annotations/questions
    if is_parsed_qa:
    ...
    # Detect annotation format: orbit anchor uses image0_path/image2_path/target_path,
    # center view uses image1_path/image2_path/center_view_path
    if results and "image0_path" in results[0]:
        annotation_format = "orbit_anchor"
        print("Detected orbit anchor annotation format (image0_path/image2_path/target_path)")
    else:
        annotation_format = "center_view"
        print("Detected center view annotation format (image1_path/image2_path/center_view_path)")

    # Filter out samples with missing image paths
    valid_results = []
    for r in results:
        if annotation_format == "orbit_anchor":
            image1_path = remap_path(r.get("image0_path"))
            image2_path = remap_path(r.get("image2_path"))
            center_view_path = remap_path(r.get("target_path"))
        else:
            image1_path = remap_path(r.get("image1_path"))
            image2_path = remap_path(r.get("image2_path"))
            center_view_path = remap_path(r.get("center_view_path"))
        if center_view_path is None and not args.omit_center_view:
            original_metadata = r.get("original_metadata", {})
            center_view_path = remap_path(original_metadata.get("center_view_output_path"))

        required_paths = [image1_path, image2_path]
        if not args.omit_center_view:
            required_paths.append(center_view_path)
    ...
    if args.shard is not None:
        try:
            ...
            # Sort by sample_id for consistent sharding across runs
    ...
    for idx, sample in enumerate(tqdm(results, desc="Generating center views")):
        sample_id = sample.get("sample_id", f"sample_{idx}")

        if annotation_format == "orbit_anchor":
            image1_path = remap_path(sample.get("image0_path"))
            image2_path = remap_path(sample.get("image2_path"))
            center_view_path = remap_path(sample.get("target_path"))
        else:
            image1_path = remap_path(sample.get("image1_path"))
            image2_path = remap_path(sample.get("image2_path"))
            center_view_path = remap_path(sample.get("center_view_path"))
        if center_view_path is None and not args.omit_center_view:
        ...
        if is_parsed_qa:
            ...
            annotation = sample.get("annotation", "").strip()
            instruction = system_prompt + "\n" + annotation + " Generate the view we are facing. "

        try:
            img1 = Image.open(image1_path).convert("RGB")
            img2 = Image.open(image2_path).convert("RGB")
            gt_center = None
            if not args.omit_center_view and center_view_path:
                gt_center = Image.open(center_view_path).convert("RGB")

            # Build input: [image1, image2, instruction text]
            ...
            # Save the last generated image as the center view prediction
            if generated_images:
                pred_image = generated_images[-1]
                pred_path = os.path.join(generated_dir, f"{sample_id}.png")
                pred_image.save(pred_path)

                # Also save all intermediate generated images if there are multiple
            ...
            # Save ground truth and inputs for comparison
            gt_path = None
            if gt_center is not None:
                gt_path = os.path.join(gt_dir, f"{sample_id}.png")
                gt_center.save(gt_path)
    ...
    print(f"\n{'='*60}")
    print(f"Center View Generation Test Complete!")
