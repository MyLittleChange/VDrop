...
class TrainingArguments:
    ...
    exit_after_save: bool = field(
        default=True,
        metadata={"help": "If True (default), exit cleanly after the next checkpoint save so a chained sbatch slot can release the queue as soon as a ckpt lands. Set to False for single-job runs that should train through the full walltime."}
    )
    ...
    use_lora: bool = field(
        default=False,
        metadata={"help": "Enable LoRA finetuning on the language model. Base weights are frozen; only low-rank adapters are trained."}
    )
    lora_rank: int = field(
        default=32,
        metadata={"help": "LoRA rank (r). Typical values: 8, 16, 32, 64."}
    )
    lora_alpha: int = field(
        default=64,
        metadata={"help": "LoRA scaling factor (alpha). Typically 2x rank."}
    )
    lora_dropout: float = field(
        default=0.05,
        metadata={"help": "Dropout probability applied to LoRA adapter inputs."}
    )
    lora_target_regex: str = field(
        default=r".*\.self_attn\.(q|k|v|o)_proj$|.*\.mlp\.(gate|up|down)_proj$",
        metadata={"help": "Regex selecting which Linear modules to wrap with LoRA. Default targets Qwen2MoT understanding path only (not *_moe_gen variants)."}
    )
...
def main():
    ...
    # CPU-side process group for object collectives (e.g. gather_object over
    # data_status). Routing those through NCCL crashes with "unhandled cuda error"
    # when the GPU is near its memory high-water mark, since gather_object has to
    # allocate CUDA staging tensors for the pickled payload.
    ...
    if training_args.use_lora:
        # LoRA flow:
        #   1. Load the frozen BAGEL base from disk
        #   2. Inject LoRA in-place (keeps model structure intact so Bagel's
        #      self.language_model.model.embed_tokens(...) access still works)
        #   3. Unfreeze new embedding rows / lm_head (special tokens added via
        #      resize_token_embeddings won't train otherwise)
        #   4. If resuming a previous LoRA run, load the saved adapter on top
        # EMA is skipped under LoRA because EMA-of-adapters offers no benefit and
        # would duplicate the frozen base (~4 GB/GPU wasted).
        adapter_ckpt = None
        base_ckpt = resume_from
        base_finetune_from_ema = finetune_from_ema
        if (resume_from is not None and os.path.exists(resume_from)
                and os.path.exists(os.path.join(resume_from, "lora_adapter.safetensors"))):
            adapter_ckpt = resume_from
            base_ckpt = training_args.resume_from
            # Base BAGEL load is a fresh load from the original model regardless
            # of whether a previous LoRA adapter exists, so honor the user's
            # --finetune_from_ema flag (auto_resume hard-codes it to False above,
            # which would break here since BAGEL ships ema.safetensors only).
            base_finetune_from_ema = training_args.finetune_from_ema
        model, _ = FSDPCheckpoint.try_load_ckpt(
            base_ckpt, logger, model, ema_model=None, resume_from_ema=base_finetune_from_ema
        )
        # Freeze all params first, then inject LoRA (which marks its own as trainable)
        ...
        lora_config = LoraConfig(
            ...
            target_modules=training_args.lora_target_regex,
            bias="none",
        )
        model.language_model = inject_adapter_in_model(lora_config, model.language_model)
        # Unfreeze embed_tokens + lm_head so new special tokens (added via
        # resize_token_embeddings) actually train. Cost: ~0.4 GB/GPU optimizer state.
        ...
        if adapter_ckpt is not None:
            model = FSDPCheckpoint.try_load_lora_adapter(adapter_ckpt, logger, model)
        if dist.get_rank() == 0:
            trainable = sum(p.numel() for p in model.parameters() if p.requires_grad)
            total = sum(p.numel() for p in model.parameters())
            logger.info(f"LoRA enabled. Trainable params: {trainable:,} / {total:,} ({100*trainable/total:.2f}%)")
        ema_model = None
    else:
    ...
    if resume_model_only:
        ...
        # Try to restore cumulative_samples from checkpoint
        cumulative_samples = 0
        if resume_from is not None and os.path.exists(resume_from):
            train_meta_path = os.path.join(resume_from, "train_meta.pt")
            if os.path.exists(train_meta_path):
                train_meta = torch.load(train_meta_path, weights_only=True, map_location="cpu")
                cumulative_samples = train_meta.get("cumulative_samples", 0)
                logger.info(f"Restored cumulative_samples={cumulative_samples} from checkpoint.")
    ...
    for curr_step, data in enumerate(train_loader, start=train_step):
        ...
        # Track cumulative samples and epoch
        step_samples = torch.tensor(len(data['sample_lens']), device=device)
        dist.all_reduce(step_samples, op=dist.ReduceOp.SUM)
        cumulative_samples += step_samples.item()

        # Extract epoch from data_indexes (min across samples in batch)
        batch_epochs = [item.get('epoch', 0) for item in data_indexes if 'epoch' in item]
        local_epoch = min(batch_epochs) if batch_epochs else 0
        local_epoch_tensor = torch.tensor(local_epoch, device=device)
        dist.all_reduce(local_epoch_tensor, op=dist.ReduceOp.MIN)
        current_epoch = local_epoch_tensor.item()
        ...
        if curr_step % training_args.log_every == 0:
            ...
            message = f"(step={curr_step:07d}, epoch={current_epoch}, samples={cumulative_samples}) "
            wandb_log = {}
            ...
            wandb_log['total_samples'] = step_samples.item()
            wandb_log['cumulative_samples'] = cumulative_samples
        ...
        if curr_step > 0 and curr_step % training_args.save_every == 0:
            if dist.get_rank() == 0:
                ...
                gather_list = None
            dist.gather_object(data_status, gather_list, dst=0, group=gloo_group)

            # Save training metadata (cumulative samples, epoch) alongside checkpoint
            if dist.get_rank() == 0:
                save_path = os.path.join(training_args.checkpoint_dir, f"{curr_step:07d}")
                os.makedirs(save_path, exist_ok=True)
                torch.save(
                    {"cumulative_samples": cumulative_samples, "epoch": current_epoch},
                    os.path.join(save_path, "train_meta.pt"),
                )
            ...
            if training_args.exit_after_save:
                logger.info(
                    f"exit_after_save=True: checkpoint saved at step {curr_step}, "
                    "exiting cleanly so the next chained slot can start."
                )
                break
