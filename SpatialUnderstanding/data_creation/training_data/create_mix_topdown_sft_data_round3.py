#!/usr/bin/env python3
"""
Create visual_only SFT training data using Round 3 photoreal Blender top-downs
as the bridge image.

Source of truth:
    /path/to/scratch/infinigen/training_data_mix_all_balance/
        no_thinking/no_thinking.jsonl
This jsonl is the canonical sample list (ShareGPT-style: conversations[],
image[], id). We strictly use these samples — no other dataset files are
consulted for sample selection. We only swap in the Round 3 topdown PNG as
the third image.

Per-row layout (3 images, visual_only_think format):
    image_list:        [cam0_bytes, cam1_bytes, topdown_blender_bytes]
    instruction_list:  [VISUAL_ONLY_THINK_SYSTEM_PROMPT + "\n" + question]
    output_text_list:  ["<image_start>", "<image_end><answer>X</answer>"]

Round 3 topdown lookup (one PNG per scene, shared across all samples of that scene):
    /path/to/scratch/infinigen/map_questions_topdown_blender/
        <scene_id>/topdown_blender.png

Cam image lookup:
    image[] paths in the jsonl are relative to /network/scratch/, so we
    prefix with /network/scratch/ to absolutize.

Usage:
    python create_mix_topdown_sft_data_round3.py
    python create_mix_topdown_sft_data_round3.py --output_dir /path/to/output
"""
...
# Round 3 topdown root (one PNG per scene_id)
...
# Default output
DEFAULT_OUTPUT_DIR = (
    "/path/to/scratch/infinigen/training_data_topdown_round3/visual_only"
)
...
def process_row(row: dict, topdown_root: str, image_root: str):
    ...
    try:
        ...
        topdown_path = os.path.join(topdown_root, scene_id, "topdown_blender.png")
...
def main():
    ...
    parser.add_argument("--topdown_root", default=DEFAULT_TOPDOWN_ROOT,
                        help="Round 3 topdown PNG root: <root>/<scene_id>/topdown_blender.png")
