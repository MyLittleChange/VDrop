...
class Bagel(PreTrainedModel):
    ...
    def generate_image(
        ...
        for i, t in tqdm(enumerate(timesteps), total=len(timesteps)):
            if debug_attn_callback is not None:
                debug_attn_callback(i)
    ...
    def generate_text(
        ...
        while step < max_length:
            if debug_attn_callback is not None:
                debug_attn_callback(step)

            generated_sequence.append(curr_tokens)
            query_token_ids = curr_tokens.detach().cpu().tolist()
            kv_lens_before = key_values_lens.detach().cpu().tolist()
            ...
            query_kv_positions = packed_query_indexes.detach().cpu().tolist()
            ...
            if return_token_trace:
                token_trace.append({
                    ...
                    "predicted_token_ids": curr_tokens.detach().cpu().tolist(),
                    "query_kv_positions": query_kv_positions,
                    "kv_lens_before": kv_lens_before,
                })
        ...
        generated = torch.stack([i.to(output_device) for i in generated_sequence], dim=0)
        if return_token_trace:
            return generated, token_trace
        return generated
