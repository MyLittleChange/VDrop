#!/bin/bash
#SBATCH --job-name=ibm_direction_visual_only_
#SBATCH --output=/path/to/ThinkMorph-BAGEL-release/SpatialUnderstanding/bagel/bagel_spatial_4gpu_direction_visual_only_bridge_masked_lora_7k_inference_bridge_mask_output.txt
#SBATCH --error=/path/to/ThinkMorph-BAGEL-release/SpatialUnderstanding/bagel/bagel_spatial_4gpu_direction_visual_only_bridge_masked_lora_7k_inference_bridge_mask_error.txt
#SBATCH --ntasks=1
...
#
# Inference-time bridge-necessity probe.
# Task: direction | Model: visual_only_bridge_masked_lora_7k
# The model generates the bridge image as usual, then its KV cache slice is
# zeroed so answer tokens cannot attend to the bridge during answer decoding.
#
set -euo pipefail
...
OUTPUT_DIR="${OUTPUT_DIR:-/path/to/scratch/VisualCoT/BAGEL_format_mix_all_balance_visual_only_bridge_masked_lora_7k_mcqs_relative_direction_normalized_inference_bridge_mask}"
...
echo "=============================================="
echo "BAGEL Spatial — inference_bridge_mask probe (direction)"
