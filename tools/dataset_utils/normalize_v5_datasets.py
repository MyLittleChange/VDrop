#!/usr/bin/env python3
"""
Normalize V5 filtered QA datasets to always reference "second image" perspective,
and add topdown_path/panorama_path fields (set to None, filled after rendering).

Processes all 5 V5 filtered datasets from spatial/ scenes:
  - dataset_counting_questions_filtered_V5.json
  - dataset_anchor_questions_filtered_V5.json
  - dataset_spatial_questions_filtered_V5.json
  - dataset_relative_distance_questions_filtered_V5.json
  - dataset_perspective_taking_questions_filtered_V5.json

Usage:
    python tools/dataset_utils/normalize_v5_datasets.py
    python tools/dataset_utils/normalize_v5_datasets.py --input_dir /path/to/dir --output_dir /path/to/out
"""
...
DEFAULT_DIR = "/path/to/scratch/infinigen/spatial"
...
def main():
    parser = argparse.ArgumentParser(description="Normalize V5 filtered QA datasets")
    parser.add_argument(
        "--input_dir",
        default=DEFAULT_DIR,
        help="Directory containing dataset_*_filtered_V5.json files",
    )
    parser.add_argument(
        "--output_dir",
        default=DEFAULT_DIR,
    ...
    for filename in V5_DATASETS:
        input_path = os.path.join(args.input_dir, filename)
        output_filename = filename.replace("_V5.json", "_V5_normalized.json")
