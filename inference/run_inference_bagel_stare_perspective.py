#!/usr/bin/env python3
"""
Run inference on STARE perspective subset using BAGEL model.

Each sample has two input images:
  - Image 1: a top-down view of a 3D scene with a marked camera position
  - Image 2: a labeled strip with 4 candidate first-person views (A/B/C/D)

The model must pick which first-person view (A/B/C/D) corresponds to the marked
camera in the top-down view.
...
PERSPECTIVE_TASK_PROMPT = (
    "You are given two images.\n"
    "Image 1 is a top-down view of a 3D indoor scene. A small red rectangle marks a camera "
    "position and the direction it is facing.\n"
    "Image 2 is a strip showing four candidate first-person views, labeled A, B, C, and D from "
    "left to right.\n\n"
    "From the marked camera position in Image 1, which first-person view (A, B, C, or D) does "
    "the camera see?\n"
    "Choose exactly one of A, B, C, D."
)
...
def calculate_accuracy(predicted: Optional[str], correct_letter: str) -> float:
    if predicted is None:
        return 0.0
    return 1.0 if predicted == correct_letter.upper() else 0.0
...
class CheckpointManager:
        ...
        if os.path.exists(self.checkpoint_path):
            try:
                with open(self.checkpoint_path, "r") as f:
                    ...
                    valid = [r for r in all_results if r.get("final_answer_text", "").strip()]
                    self.results = {r["_idx"]: r for r in valid}
                    print(f"Loaded checkpoint with {len(self.results)} completed samples")
                    dropped = len(all_results) - len(valid)
                    if dropped > 0:
                        print(f"Filtered out {dropped} samples with empty responses (will recompute)")
                    return self.results
    ...
    def add_result(self, result: Dict):
        with self.lock:
            self.results[result["_idx"]] = result
            self.completed_count += 1
            if self.completed_count % self.save_interval == 0:
                self._save_checkpoint()

    def _save_checkpoint(self):
        data = {"results": list(self.results.values()), "total_completed": len(self.results)}
...
class BAGELStarePerspectiveInference:
    def __init__(
        self,
        model_path: str,
    ...
    def run_single_sample(
        self,
        topdown_image: Image.Image,
        options_image: Image.Image,
        correct_letter: str,
        think: bool = True,
        thinking_mode: str = "visual_only_thinking",
        sample_id: str = "",
        ...
        if thinking_mode == "zebra_cot":
            full_prompt = ZEBRA_COT_SYSTEM_PROMPT + "\n" + PERSPECTIVE_TASK_PROMPT
        elif think:
            full_prompt = PERSPECTIVE_TASK_PROMPT
        else:
        ...
        for retry in range(NUM_RETRIES):
            try:
                if thinking_mode == "zebra_cot":
                    ...
                    current_input = [topdown_image, options_image, full_prompt]
                    output_list = []
                    ...
                    input_list = [topdown_image, options_image, full_prompt]
                ...
                for out_item in output_list:
                    if isinstance(out_item, str):
                        ...
                        if output_dir is not None:
                            ...
                            ipath = os.path.join(output_dir, fname)
                            out_item.save(ipath)
                            saved_image_paths.append(ipath)
                        image_round += 1

                model_output_result = model_output_result.strip()
                predicted = extract_answer(model_output_result)
                acc = calculate_accuracy(predicted, correct_letter)
...
def decode_image(img_dict) -> Image.Image:
    return Image.open(io.BytesIO(img_dict["bytes"])).convert("RGB")


def main():
    parser = argparse.ArgumentParser(description="Run inference on STARE perspective subset using BAGEL")
    parser.add_argument(
        "--data_file",
    ...
    parser.add_argument("--model_path", required=True, help="Path to BAGEL model directory")
    ...
    parser.add_argument(
        "--output_file",
        default="/path/to/scratch/VisualCoT/stare_perspective/BAGEL/inference_results_bagel.json",
    )
    parser.add_argument("--generated_images_dir", default=None)
    parser.add_argument("--num_samples", type=int, default=None)
    parser.add_argument("--random_seed", type=int, default=42)
    parser.add_argument("--shard", type=str, default=None,
                        help="Shard specification 'index/total' (e.g., '0/1' for single-shard)")
    ...
    parser.add_argument("--image_shapes", type=int, nargs=2, default=None,
                        help="(H, W) shape for generated visual-thinking images.")
    parser.add_argument(
    ...
    if args.shard is not None:
        try:
            ...
            all_data = sorted(all_data, key=lambda x: x.get("qid", ""))
            total = len(all_data)
            ...
            print(f"Shard {shard_idx}/{total_shards}: processing samples {start}-{end-1} ({len(all_data)} samples)")
        except ValueError as e:
            print(f"Error parsing shard '{args.shard}': {e}")
            return
    else:
        shard_idx = 0
    ...
    if not args.no_resume:
        if os.path.exists(checkpoint_path):
            ...
            in_shard = len(completed_ids & shard_ids)
            print(f"Auto-resuming from checkpoint: {in_shard} completed, {len(all_data) - in_shard} remaining")
    ...
    inference_engine = BAGELStarePerspectiveInference(
    ...
    def process_single_sample(info: Dict[str, Any]):
        idx_key = info["_idx"]
        qid = info["qid"]
        correct_letter = info["answer"]
        try:
            imgs = info["images"]
            if len(imgs) < 2:
                print(f"Skipping {qid}: expected 2 images, got {len(imgs)}")
                return None
            topdown = decode_image(imgs[0])
            options = decode_image(imgs[1])
        except Exception as e:
            print(f"Error decoding images for {qid}: {e}")
            return None
        ...
        try:
            ...
            result["other_info"] = info.get("other_info", "")
            return result
        except Exception as e:
    ...
    try:
        for info in tqdm(samples_to_process, desc="Running inference"):
            ...
            if len(all_results) > 0 and len(all_results) % 10 == 0:
                acc = sum(r["accuracy"] for r in all_results) / len(all_results)
                print(f"Current accuracy: {acc:.4f} ({len(all_results)} samples)")
